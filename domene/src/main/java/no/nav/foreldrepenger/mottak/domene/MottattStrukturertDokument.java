...
public abstract class MottattStrukturertDokument<S> {
    ...
    public static MottattStrukturertDokument toXmlWrapper(Object skjema) {
        if (skjema instanceof SoeknadsskjemaEngangsstoenad) { // NOSONAR
        ...
        if (skjema instanceof Soeknad) { // NOSONAR  Dto plukker ut info for foreldrepenger, engangsstønad og endringssøknad
            return new Søknad((Soeknad) skjema);
        }
        if (skjema instanceof no.nav.vedtak.felles.xml.soeknad.v2.Soeknad) { // NOSONAR Dto plukker ut info for foreldrepenger, engangsstønad og endringssøknad
